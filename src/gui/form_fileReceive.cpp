...
form_fileReceive::form_fileReceive(CFileTransferReceive &FileReceive)
  ...
  connect(&FileReceive, SIGNAL(signAlreadyReceivedSizeChanged(quint64)), this,
...
void form_fileReceive::init() {
  ...
  progressBar->setValue(FileReceive.getAlreadyReceivedSize());
...
void form_fileReceive::start() {
  if (FileReceive.checkIfAlreadyAcceptTheRequest() == false) {
    askTheUser();
  }
}
